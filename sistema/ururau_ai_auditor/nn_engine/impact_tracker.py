...
class ImpactTracker:
    ...
    def _extrair_metricas(self, db_path):
        ...
        try:
            ...
            row = cur.fetchone()
            metricas = {"pautas_24h": row[0] or 0, "avg_score_24h": row[1] or 0, "urgentes_24h": row[2] or 0}
    ...
    def registrar_baseline(self, patch_id, descricao):
        db = self.root / "sistema" / "data" / "ururau.db"
        metricas = self._extrair_metricas(db)
        self._registros.append({"patch_id": patch_id, "descricao": descricao, "fase": "baseline", "timestamp": None, "metricas": metricas})
        self._save()

    def registrar_fechamento(self, patch_id):
        db = self.root / "sistema" / "data" / "ururau.db"
        ...
        resultado = {"patch_id": patch_id, "fase": "fechamento", "metricas_pre": metricas_pre, "metricas_pos": metricas_pos, "delta_percent": delta, "melhorou": sum(1 for d in delta.values() if d > 0) > sum(1 for d in delta.values() if d < 0)}
        self._registros.append(resultado)
        self._save()
        return resultado

    def get_historico(self, patch_id=None):
        if patch_id:
