...
Armazena: problema -> solucao -> resultado.
Permite busca por similaridade para reaproveitar correções.
"""
...
class LongTermMemory:
    """Memória de correções aplicadas com busca semântica."""
    ...
    def adicionar(self, problema: str, solucao: str, resultado: str, arquivo: str, patch_id: str = None) -> str:
        """Armazena uma correção na memória."""
        mid = patch_id or str(uuid.uuid4())
        embedding = self._vec.encode(problema).tolist()
        self._memorias.append({
            "id": mid,
            "problema": problema,
            "solucao": solucao,
            "resultado": resultado,
    ...
    def buscar(self, problema_novo: str, top_k: int = 3) -> List[Dict]:
        """Busca correções similares por cosseno de embedding."""
    ...
    def marcar_utilizada(self, memoria_id: str):
        for m in self._memorias:
            if m["id"] == memoria_id:
                m["utilizada"] += 1
                self._save()
