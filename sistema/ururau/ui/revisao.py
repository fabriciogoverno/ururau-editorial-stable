...
_STATUS_REVISAO = {'revisada', 'bloqueada', getattr(StatusPauta, 'EM_REDACAO', 'em_redacao')}
_STATUS_PAUTA_REVISAO = {
    'revisada', 'bloqueada',
    getattr(StatusPauta, 'EM_REDACAO', 'em_redacao'), 'pronta',  # pronta mas reprovada
}
...
def _e_para_revisao(pauta: dict) -> bool:
    ...
    if status in ('bloqueada', 'revisada'):
        return True
    if md.get("auditoria_bloqueada"):
        return True
    ...
    if status == 'pronta' and _erros_validacao(md):
        return True
    return False
