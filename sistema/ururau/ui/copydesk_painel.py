...
def gerar_sugestoes_item(md: dict, fonte_texto: str = "",
    ...
    if not retranca:
        ...
        primeiros_3 = " ".join(retranca.split()[:1])
...
def regenerar_sugestao_ia(campo: str, valor_atual: str, contexto: dict,
    ...
    try:
        ...
        instrucoes = {
            ...
            "retranca": (
                "Escreva uma RETRANCA editorial de uma palavra "
...
def gerar_sugestoes_paragrafos(corpo: str) -> list[dict]:
    """
    v132: o corpo passa a ser revisado como texto único no CopyDesk.

    Antes o painel quebrava o corpo parágrafo a parágrafo, o que deixava a revisão
    lenta e confusa. Agora preserva os parágrafos dentro do mesmo campo editável.
    """
    ...
    if "—" in corpo or "–" in corpo:
        problemas.append("contém travessão; substituir por vírgula, dois-pontos ou ponto")
    for expr in [
        "vale lembrar", "é importante destacar", "e importante destacar", "cabe ressaltar",
        "em meio a", "cenário complexo", "cenario complexo", "nesse contexto",
        "a população aguarda", "a populacao aguarda", "novas informações serão divulgadas",
        "novas informacoes serao divulgadas", "as investigações seguem", "as investigacoes seguem",
    ]:
        if expr in low:
            problemas.append(f"expressão genérica: '{expr}'")
            break
    if len(corpo) < 900:
        problemas.append(f"corpo curto ({len(corpo)} chars); verificar fonte original antes de publicar")
    if "\n\n" not in corpo and len(corpo) > 1000:
        problemas.append("texto longo sem quebras de parágrafo visíveis")
...
class JanelaCopydeskItem(tk.Toplevel if _TK_DISPONIVEL else object):
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="[IA] Reescrever pela Fonte",
                  command=self._regenerar_materia_pela_fonte_v87,
        ...
        if self._paragrafos:
            ...
            sep.pack(fill="x")
            tk.Label(self._lista_frame, text="Corpo da matéria",
    ...
    def _selecionar_paragrafo(self, idx: int):
        """Mostra editor do corpo completo."""
        ...
        tk.Button(af, text="Salvar corpo",
                  command=_salvar_par, bg=COR_VERDE, fg="white",
                  relief="flat", padx=8, pady=4, cursor="hand2",
                  font=("Helvetica", 9, "bold")).pack(side="left", padx=4)
        tk.Button(af, text="Limpar corpo",
                  command=_remover_par, bg=COR_VERMELHO, fg="white",
                  relief="flat", padx=8, pady=4, cursor="hand2",
                  font=("Helvetica", 9, "bold")).pack(side="left", padx=4)

    def _aplicar(self, chave: str, acao: str):
        """Aplica decisao do editor a um campo revisavel."""
        sug = self._sugestoes.get(chave, {})
        valor_anterior = self._md_atual.get(chave, sug.get("valor_atual", ""))
        if acao == "accepted":
            novo = sug.get("sugestao", "")
        elif acao == "edited":
            novo = getattr(self, "_edit_box", None).get("1.0", "end").strip() if getattr(self, "_edit_box", None) else valor_anterior
        elif acao == "rejected":
            novo = sug.get("valor_atual", valor_anterior)
        else:  # ok
            novo = self._md_atual.get(chave, sug.get("valor_atual", ""))

        self._md_atual[chave] = novo
        sug["status"] = acao
        sug["valor_atual"] = novo
        sug["caracteres_atual"] = len(str(novo or ""))
        ...
        try:
            self._status_bar.config(text=f"Campo {chave} atualizado: {acao}.")
        except Exception:
            pass

    def _reconstruir_corpo(self):
        """Reconstrói corpo_materia/conteudo a partir dos parágrafos editados."""
        corpo = "\n\n".join(
            (p.get("texto") or "").strip()
            for p in self._paragrafos
            if p.get("status") != "removed" and (p.get("texto") or "").strip()
        )
        self._md_atual["corpo_materia"] = corpo
        self._md_atual["conteudo"] = corpo
        self._historico.append({
            "campo": "corpo_materia",
            "acao": "rebuild_paragraphs",
            "depois_chars": len(corpo),
            "quando": datetime.now().isoformat(timespec="seconds"),
        })

    def _refresh_copydesk_v87(self):
        try:
            self._sugestoes = gerar_sugestoes_item(self._md_atual, client=self._client, modelo=self._modelo)
        except Exception as e:
            print(f"[COPYDESK v87] Erro atualizando sugestões: {e}")
            self._sugestoes = {}
        try:
            self._paragrafos = gerar_sugestoes_paragrafos(self._md_atual.get("corpo_materia") or self._md_atual.get("conteudo") or "")
        except Exception as e:
            print(f"[COPYDESK v87] Erro atualizando parágrafos: {e}")
            self._paragrafos = []
        ...
        self._build_ui()
        try:
            self._status_bar.config(text="[v87] Matéria regenerada com base no texto da aba Fonte. Revise e clique Salvar mudanças.")
        except Exception:
            pass

    def _regenerar_materia_pela_fonte_v87(self):
        try:
            from ururau.editorial.copydesk_regenerador_v87 import extrair_texto_fonte_copydesk
            fonte = extrair_texto_fonte_copydesk(self._md_atual, self._pauta)
        except Exception as e:
            messagebox.showerror("Copydesk v87", f"Não foi possível localizar a fonte extraída: {e}", parent=self)
            return

        corpo_atual = self._md_atual.get("corpo_materia") or self._md_atual.get("conteudo") or ""
        if len(fonte) < 800:
            messagebox.showwarning("Copydesk v87", f"A fonte extraída tem só {len(fonte)} caracteres. O botão precisa de fonte longa para reescrever com segurança.", parent=self)
            return

        msg = (f"A fonte extraída tem {len(fonte)} caracteres e o corpo atual tem {len(corpo_atual)} caracteres.\n\n"
               "O GPT-4.1-mini vai reescrever a matéria usando APENAS o texto da aba Fonte.\n"
               "Depois disso, revise e clique em 'Salvar mudanças'.\n\nContinuar?")
        if not messagebox.askyesno("Reescrever matéria pela Fonte", msg, parent=self):
            return
        try:
            self._status_bar.config(text="[v87] Reescrevendo matéria com GPT-4.1-mini usando a Fonte extraída...")
            self.update_idletasks()
        except Exception:
            pass
        ...
        def _worker():
            try:
                ...
                novo = regenerar_materia_com_fonte_v87(self._md_atual, self._pauta, client=self._client, modelo=self._modelo or "gpt-4.1-mini")
                def _ok():
                    antes_chars = len(corpo_atual or "")
                    depois_chars = len(novo.get("corpo_materia") or novo.get("conteudo") or "")
                    preservados = {}
                    for k in ("imagem", "imagem_final", "caminho_imagem", "foto", "foto_final", "score_risco", "termos_ia_detectados", "status", "slug", "cleaned_source_text", "raw_source_text", "original_source_text"):
                        if self._md_atual.get(k) and not novo.get(k):
                            preservados[k] = self._md_atual.get(k)
                    self._md_atual.update(novo)
                    self._md_atual.update(preservados)
                    self._md_atual["conteudo"] = self._md_atual.get("corpo_materia") or self._md_atual.get("conteudo") or ""
                    self._md_atual["corpo_materia"] = self._md_atual["conteudo"]
                    self._historico.append({"campo": "materia_completa", "acao": "regenerar_pela_fonte_v87", "antes_chars": antes_chars, "depois_chars": depois_chars, "fonte_chars": len(fonte), "modo": self._md_atual.get("modo_copydesk_v87", ""), "quando": datetime.now().isoformat(timespec="seconds")})
                    self._refresh_copydesk_v87()
                    messagebox.showinfo("Copydesk v87", f"Matéria regenerada pela Fonte.\nAntes: {antes_chars} chars\nDepois: {depois_chars} chars\nFonte: {len(fonte)} chars", parent=self)
                self.after(0, _ok)
            except Exception as e:
                msg_erro = str(e)
                def _err():
                    try:
                        self._status_bar.config(text=f"[v87] Falha ao reescrever pela Fonte: {msg_erro}")
                    except Exception:
                        pass
                    messagebox.showerror("Copydesk v87", msg_erro, parent=self)
                self.after(0, _err)
        threading.Thread(target=_worker, daemon=True).start()

    def _revalidar(self):
        """Reexecuta validações locais básicas sem chamar o CMS."""
        problemas = []
        titulo = self._md_atual.get("titulo_seo") or self._md_atual.get("titulo") or ""
        capa = self._md_atual.get("titulo_capa") or ""
        if len(titulo) > 89:
            problemas.append(f"Título SEO acima de 89 caracteres ({len(titulo)}).")
        if capa and len(capa) > 60:
            problemas.append(f"Título de capa acima de 60 caracteres ({len(capa)}).")
        corpo = self._md_atual.get("corpo_materia") or self._md_atual.get("conteudo") or ""
        if len(corpo.strip()) < 300:
            problemas.append("Corpo da matéria muito curto ou vazio.")
        if "—" in corpo or "—" in titulo:
            problemas.append("Há travessão no texto; substituir por vírgula, dois-pontos ou parênteses.")

        msg = "Revalidação OK." if not problemas else "\n".join(problemas[:8])
        try:
            messagebox.showinfo("Revalidação", msg, parent=self)
            self._status_bar.config(text=msg.split("\n")[0])
        except Exception:
            print(f"[COPYDESK] {msg}")

    def _salvar(self):
        """Persiste alterações no callback do painel principal e fecha a janela."""
        self._md_atual.setdefault("historico_correcoes", [])
        hist = self._md_atual.get("historico_correcoes")
        if isinstance(hist, list):
            hist.extend(self._historico)
        else:
            self._md_atual["historico_correcoes"] = list(self._historico)

        if self._on_salvar:
            try:
                self._on_salvar(self._pauta, self._md_atual, self._historico)
            except TypeError:
                try:
                    self._on_salvar(self._pauta, self._md_atual)
                except TypeError:
                    self._on_salvar(self._md_atual)
            except Exception as e:
...
__all__ = [
    "JanelaCopydeskItem",
    "gerar_sugestoes_item",
    "regenerar_sugestao_ia",
    "regenerar_sugestao_ia_full",
    "gerar_sugestoes_paragrafos",
]
