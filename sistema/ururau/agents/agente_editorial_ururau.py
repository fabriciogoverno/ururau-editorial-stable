...
retranca: uma palavra. Temática e específica. Exemplos: Política, Governo RJ, Polícia, Justiça, STF, Alerj, Economia, Trabalho, Saúde, Educação, Cidades, Esportes, Tecnologia, Brasil, Mundo.
...
□ retranca: uma palavra, específica?
...
def validate_article_output(
    ...
    _chk(len(palavras_retranca) == 1,
         f"retranca deve ter uma palavra (tem {len(palavras_retranca)}): '{retranca}'")
...
# URURAU v47.2 — matriz editorial única no agente
try:
    ...
    _L=_lim(); LIMITE_TITULO_SEO=int(_L.get('titulo_seo_max',LIMITE_TITULO_SEO)); LIMITE_TITULO_CAPA=int(_L.get('titulo_capa_max',LIMITE_TITULO_CAPA)); LIMITE_SUBTITULO=int(_L.get('subtitulo_curto_max',LIMITE_SUBTITULO)); LIMITE_LEGENDA=int(_L.get('legenda_curta_max',LIMITE_LEGENDA)); TAGS_MIN=int(_L.get('tags_min',TAGS_MIN)); TAGS_MAX=int(_L.get('tags_max',TAGS_MAX))
    EXPRESSOES_PROIBIDAS={**EXPRESSOES_PROIBIDAS, **_exp()}; FRASES_GENERICAS_PROIBIDAS=_fg() or FRASES_GENERICAS_PROIBIDAS; SYSTEM_PROMPT_EDITORIAL_URURAU=(SYSTEM_PROMPT_EDITORIAL_URURAU.rstrip()+'\n\n'+_bp()).strip()
