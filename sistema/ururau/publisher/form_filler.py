...
async def _screenshot_debug(page: "Page", nome: str):
    """v81: salva screenshot e HTML para diagnóstico de falhas."""
    try:
        _PASTA_DEBUG.mkdir(exist_ok=True)
        ts = int(time.time())
        caminho = _PASTA_DEBUG / f"debug_{nome}_{ts}.png"
        await page.screenshot(path=str(caminho), full_page=True)
        print(f"[v81][FORM] Screenshot salvo: {caminho}")
        try:
            html_path = _PASTA_DEBUG / f"debug_{nome}_{ts}.html"
            html = await page.content()
            html_path.write_text(html[:150000], encoding="utf-8", errors="replace")
            print(f"[v81][FORM] HTML salvo: {html_path}")
        except Exception:
            pass
    except Exception:
        pass
...
async def preencher_e_publicar(
    ...
    try:
        ...
        # v103: para matérias geradas pelo robô, o campo Nome da fonte no CMS
        # deve ser sempre Redação. O link da fonte original continua preenchido
        # abaixo em linkfonte.
        nomefonte = "Redação"
        ...
        # ── Rascunho v82 — controla o checkbox real do painel Ururau ─────────
        # RASCUNHO NO URURAU = marcar:
        # "Não publicar a notícia agora. Salvar como rascunho!"
        # Depois clicar em Cadastrar.
        try:
            cb_status = await page.query_selector('input[name="status"][type="checkbox"]')

            # Fallback robusto: localizar checkbox por texto/label contendo "Não publicar" ou "rascunho".
            if not cb_status:
                cb_status = await page.evaluate_handle("""() => {
                    ...
                    for (const cb of checks) {
                        ...
                        const parent = cb.closest('label, div, p, span, li, tr, td');
                        if (parent) txt += ' ' + parent.innerText;
                        const n = norm(txt);
                        if (n.includes('nao publicar') || n.includes('rascunho') || n.includes('salvar como rascunho')) {
                            return cb;
                        }
                    }
                    return null;
                ...
                try:
                    el_handle = await cb_status.as_element()
                    cb_status = el_handle if el_handle else None
                except Exception:
                    cb_status = None

            if cb_status:
                # v85: ElementHandle real. Garante o estado com check/uncheck e valida via JS.
                marcado = await cb_status.is_checked()
                if rascunho and not marcado:
                    await cb_status.check(force=True)
                    print("[v85][FORM] Checkbox de rascunho marcada: Não publicar a notícia agora. Salvar como rascunho!")
                elif not rascunho and marcado:
                    await cb_status.uncheck(force=True)
                    print("[v85][FORM] Checkbox de rascunho desmarcada: publicação direta autorizada pelo Preview.")
                else:
                    estado = "marcada (rascunho no painel)" if marcado else "desmarcada (publicação direta)"
                    print(f"[v85][FORM] Checkbox de rascunho já estava {estado}.")

                try:
                    estado_final = await page.evaluate("""(cb) => !!cb.checked""", cb_status)
                    print(f"[v85][FORM] Estado final checkbox rascunho: {estado_final} | rascunho_solicitado={rascunho}")
                    if bool(estado_final) != bool(rascunho):
                        await page.evaluate("""(args) => {
                            const cb = args[0], checked = args[1];
                            cb.checked = checked;
                            cb.dispatchEvent(new Event('input', {bubbles:true}));
                            cb.dispatchEvent(new Event('change', {bubbles:true}));
                        }""", [cb_status, bool(rascunho)])
                        estado_final = await page.evaluate("""(cb) => !!cb.checked""", cb_status)
                        print(f"[v85][FORM] Estado final checkbox após JS: {estado_final}")
                except Exception as _e:
                    print(f"[v85][FORM] Validação JS checkbox falhou: {_e}")
            else:
                print("[v85][FORM] Checkbox de rascunho não encontrado. Salvando screenshot para diagnóstico.")
                await _screenshot_debug(page, "checkbox_rascunho_nao_encontrado")
                if rascunho:
                    return False
                # Em publicação direta, ausência da checkbox não bloqueia: o padrão do CMS é publicar.
                print("[v85][FORM] Publicação direta seguirá sem checkbox de rascunho localizada.")
        except Exception as e:
            print(f"[v82][FORM] Erro ao controlar checkbox de rascunho: {e}")
            await _screenshot_debug(page, "erro_checkbox_rascunho")
            if rascunho:
                return False
...
async def executar_publicacao_playwright(
    ...
    if not usuario or not senha:
        print("[FORM] Login/senha do CMS ausentes. Configure URURAU_LOGIN e URURAU_SENHA no .env.")
        return False
