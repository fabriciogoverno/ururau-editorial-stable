...
# v47: carregamento determinístico das credenciais.
# Ordem correta: exemplo só fornece defaults; env_principal e .env sobrescrevem.
# Nunca usar load_dotenv() sem caminho aqui, porque o diretório de execução do .bat
# pode variar e fazer o painel ler/salvar outro .env.
_ENV_DEFAULTS = [
    BASE_DIR / "credenciais" / ".env.exemplo",
]
_ENV_REAL = [
    BASE_DIR / "credenciais" / "env_principal.env",
    BASE_DIR / ".env",
]


def carregar_env_seguro() -> None:
    """Carrega .env de forma previsível, com prioridade para sistema/.env."""
    for _env_path in _ENV_DEFAULTS:
        try:
            if _env_path.exists():
                load_dotenv(_env_path, override=False)
        except Exception:
            pass
    for _env_path in _ENV_REAL:
        try:
            if _env_path.exists():
...
# ── URLs do CMS ───────────────────────────────────────────────────────────────
SITE_LOGIN_URL  = _str("SITE_LOGIN_URL",  os.getenv("URURAU_PANEL_URL", "https://www.ururau.com.br/acessocpainel/"))
SITE_NOVA_URL   = _str("SITE_NOVA_URL",   os.getenv("URURAU_NEW_POST_URL", "https://www.ururau.com.br/acessocpainel/noticias/nova/"))
...
URURAU_BLOQUEAR_TITULO_SOZINHO = _bool("URURAU_BLOQUEAR_TITULO_SOZINHO", True)
URURAU_MIN_CHARS_FONTE_MONITOR = _int("URURAU_MIN_CHARS_FONTE_MONITOR", 500)
...
_PLACEHOLDERS_CHAVE = {
    ...
    "coloque_sua_chave", "insira_sua_chave", "cole_sua_chave_aqui",
    "cole_sua_chave", "cole_sua_key", "your_api_key_here", "sk-XXXXXXXX",
}
...
def validate_openai_config(api_key: str = "", modelo: str = "") -> ConfigValidationResult:
    ...
    if not chave:
        err = _build_config_error(
            ...
            "OPENAI_API_KEY não configurada. A IA será desativada e o fallback local será usado.",
            "Opcional: adicione OPENAI_API_KEY=sk-... no .env para usar IA; sem chave, o sistema continua com fallback local.",
        )
    ...
    if chave.lower() in _PLACEHOLDERS_CHAVE or chave.lower().startswith("sua_") or chave.lower().startswith("cole_") or chave.lower().startswith("coloque_") or (
