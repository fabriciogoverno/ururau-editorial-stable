...
TEXTO-FONTE INTEGRAL PARA CONFERÊNCIA E REESCRITA:
{fonte_integral_v103}
...
"""
    ...
    if client is None:
        try:
            ...
            _tr = trace_fallback("copydesk_ia", modelo, "Copydesk IA sem client OpenAI; mantendo limpeza local.", origem="copydesk_sem_client")
            dados = aplicar_trace_em_dados(dados, _tr, fallback_motivo=_tr.get("erro_mensagem", ""))
        except Exception:
            dados["copydesk_ia_status"] = "fallback_sem_ia_cliente_ausente"
            dados["copydesk_ia_chamada_ok"] = False
        dados["copydesk_ia_status"] = dados.get("copydesk_ia_status") or "fallback_sem_ia_cliente_ausente"
        dados["copydesk_ia_chamada_ok"] = False
        return dados
    ...
    try:
        ...
        dados["copydesk_ia_status"] = "openai_ok"
        dados["copydesk_ia_modelo"] = modelo
        dados["copydesk_ia_chamada_ok"] = True
        try:
            from ururau.ia.diagnostico import registrar_evento_ia
            registrar_evento_ia("copydesk_ia", "openai_ok", modelo=modelo, provider="openai", mensagem="Copydesk IA respondeu com sucesso.", sucesso=True)
        except Exception:
            pass
        return dados
        ...
        try:
            ...
            _tr = trace_openai_erro("copydesk_ia", modelo, e)
            dados = aplicar_trace_em_dados(dados, _tr, fallback_motivo=_tr.get("erro_mensagem", ""))
            dados["copydesk_ia_status"] = _tr.get("status") or "openai_call_failed"
            dados["copydesk_ia_modelo"] = modelo
            dados["copydesk_ia_chamada_ok"] = False
            dados["copydesk_ia_erro"] = _tr.get("erro_mensagem", str(e))
        except Exception:
            dados["copydesk_ia_status"] = "openai_call_failed"
            dados["copydesk_ia_modelo"] = modelo
            dados["copydesk_ia_chamada_ok"] = False
            dados["copydesk_ia_erro"] = str(e)[:500]
        return dados
