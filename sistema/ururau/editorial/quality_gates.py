...
def run_autopub_copydesk(
    ...
    # 3. retranca: trim para uma palavra
    ret = art.get("retranca") or ""
    palavras_ret = ret.split()
    if len(palavras_ret) > 3:
        art["retranca"] = " ".join(palavras_ret[:1])
        alteracoes.append("retranca cortada para uma palavra")
    ...
    # URURAU v47.2: expressões/termos de IA pela matriz editorial única.
    try:
        from ururau.editorial.regras_editoriais import validar_termos_ia_em_artigo
        check_ia = validar_termos_ia_em_artigo(art, modo="monitor")
        for erro in check_ia.get("erros", []):
            residuais.append(erro)
    except Exception:
        corpo_n = _norm(corpo)
        EXPR = ("vale lembrar", "e importante destacar", "cabe ressaltar", "reforca", "acende o alerta")
        for expr in EXPR:
            if expr in corpo_n:
                residuais.append({"categoria":"EDITORIAL_BLOCKER","codigo":"expressao_proibida","mensagem":f"Corpo contem expressao proibida: '{expr}'","bloqueia_publicacao": True})
...
def safe_can_publish(article: dict, modo: str = "panel") -> tuple[bool, str]:
    ...
    try:
        ...
        return can_publish(article, modo=("monitor" if modo in ("monitor", "direct") else "panel"))
...
def monitor_autopub_check(
    ...
    try:
        ...
        motivos.append(f"auditoria_v78c falhou: {exc}")
