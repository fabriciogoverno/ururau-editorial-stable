...
def classificar_canal(titulo: str, resumo: str) -> tuple[str, str, int]:
    ...
    # ── Regras determinísticas v77b — corrigem falsos positivos críticos ─────
    # Antes do placar genérico por substring, decide casos que não podem ser
    # jogados em editoria errada no monitor 24h. Ex.: "Putin" não pode virar
    # Saúde por conter "uti"; "Congresso" não pode virar Esportes por termos
    # soltos de disputa/derrota; acidente com morte não é Política só porque a
    # fonte veio de feed político.
    if any(t in texto_low for t in [
        "congresso", "câmara dos deputados", "senado", "stf", "tse", "tre",
        "tribunal superior eleitoral", "supremo tribunal federal", "alerj",
        "governo do rj", "governo do rio", "palácio guanabara", "palacio guanabara",
        "pesquisa eleitoral", "quaest", "genial investimentos", "intenção de voto", "intenções de voto",
        "intencao de voto", "intencoes de voto", "mandato", "cassação", "cassacao",
        "alexandre de moraes", "moraes autoriza", "bolsonaro", "lula",
        "eduardo paes", "douglas ruas", "garotinho", "wilson witzel", "rodrigo bacellar",
        "eleitor", "título de eleitor", "dosimetria", "anistia", "pec ",
        "projeto de lei", "ministro do stf", "deputado", "senador"
    ]):
        if not any(t in texto_low for t in ["copa do mundo", "flamengo", "vasco", "botafogo", "fluminense", "palmeiras", "bragantino"]):
            return "Política", "alta", 8

    if any(t in texto_low for t in [
        "copa do mundo", "campeonato brasileiro", "libertadores", "copa do brasil",
        "flamengo", "vasco", "botafogo", "fluminense", "palmeiras", "bragantino",
        "atlético-mg", "atletico-mg", "chape", "chapecoense", "brasileirão"
    ]):
        return "Esportes", "alta", 8

    if any(t in texto_low for t in [
        "chanceler", "irã", "rússia", "russia", "putin", "eua",
        "estados unidos", "trump", "obama", "israel", "hamas", "ucrânia",
        "ucrania", "ataque a embarcação", "oceano pacífico", "oriente médio"
    ]):
        return "Brasil e Mundo", "alta", 8

    if any(t in texto_low for t in [
        "morre", "morreu", "morte", "acidente", "delegacia", "perícia",
        "bombeiros", "prensado", "imprensado", "resgat", "vítima", "vitima"
    ]):
        if not any(t in texto_low for t in ["morte por doença", "óbito por doença", "morre de câncer", "morre de cancer"]):
            return "Polícia", "alta", 7
...
def calcular_score_completo(
    ...
    prioridade = pauta.get("prioridade", 1)  # v100: 3<=1h, 2<=2h, 1<=4h, 0=fora da janela

    texto = (titulo + " " + resumo).lower()

    # ── 1. Canal ──────────────────────────────────────────────────────────────
    canal_auto, conf_auto, pts_auto = classificar_canal(titulo, resumo)
    if canal_f and canal_f in CANAIS_URURAU:
        # v77b: canal forçado de feed é sugestão, não sentença.
        # Se o conteúdo aponta com alta confiança para outra editoria, prevalece
        # a classificação textual para evitar erros como Congresso→Esportes,
        # Irã/Rússia→Saúde ou acidente→Política.
        if conf_auto == "alta" and canal_auto != canal_f:
            canal, confianca, pts_canal = canal_auto, conf_auto, max(pts_auto, 6)
        else:
            canal, confianca, pts_canal = canal_f, "alta", 5
    else:
        canal, confianca, pts_canal = canal_auto, conf_auto, pts_auto
    ...
    if prioridade >= 3:
        sd.score_frescor = 24
        motivos_ap.append("Publicada na última hora (prioridade máxima v100)")
    elif prioridade == 2:
        sd.score_frescor = 18
        motivos_ap.append("Publicada nas últimas 2h")
    elif prioridade == 1:
        sd.score_frescor = 10
        motivos_ap.append("Publicada dentro da janela de 4h")
    else:
        sd.score_frescor = 0
        sd.pen_horario = -25
        sd.barrado_por_tempo = True
        motivos_rej.append("Fora da janela configurada de publicação — descartada por idade")
    ...
    # ── 17c. Linha editorial v129 — Config > Termos vira score positivo ─────
    try:
        from ururau.coleta.linha_editorial_v129 import analisar_texto_linha_editorial_v129
        analise_v129 = analisar_texto_linha_editorial_v129(titulo, resumo, fonte, pauta.get("link_origem", ""))
        boost_v129 = int(analise_v129.get("boost", 0) or 0)
        termos_v129 = analise_v129.get("termos") or []
        if boost_v129 > 0:
            sd.score_editorial = max(0, min(100, sd.score_editorial + boost_v129))
            pauta["_v129_boost_linha_editorial"] = boost_v129
            pauta["_v129_termos_positivos"] = termos_v129[:18]
            pauta["_v129_motivo_linha_editorial"] = analise_v129.get("motivo", "")
            if analise_v129.get("canal") and not canal_f:
                sd.canal_sugerido = analise_v129.get("canal")
                pauta["canal_forcado"] = analise_v129.get("canal")
            motivos_ap.append(
                f"Linha editorial v129 +{boost_v129}: " + ", ".join(str(t) for t in termos_v129[:6])
            )
    except Exception:
        pass
...
# ══════════════════════════════════════════════════════════════════════════════
# URURAU v74 — score de relevância + duplicidade semântica no filtro de pautas
# ══════════════════════════════════════════════════════════════════════════════

_old_filtrar_e_ordenar_v74 = filtrar_e_ordenar

def filtrar_e_ordenar(pautas: list[dict], score_minimo: int = None, max_por_canal: int = None, modo: str = "painel", contexto_fontes: dict = None) -> list[dict]:  # type: ignore[override]
    try:
        ...
        for p in pautas:
            rel = calcular_relevancia_v74(p)
            p.update(rel)
        # marca duplicidades dentro do ciclo antes do filtro legado
        for p in pautas:
            dup = verificar_duplicidade_semantica(p, pautas)
            p["duplicidade_semantica_v74"] = round(float(dup.score), 3)
            if dup.is_duplicate:
                p["duplicidade_motivo_v74"] = dup.reason
                p["duplicidade_titulo_v74"] = dup.matched_title
                p["score_editorial"] = max(0, int(p.get("score_editorial", 0) or 0) - 25)
    except Exception as exc:
        print(f"[SCORING v74] pré-score v74 falhou sem bloquear: {exc}")
    resultado = _old_filtrar_e_ordenar_v74(pautas, score_minimo, max_por_canal, modo, contexto_fontes)
    try:
        resultado.sort(key=lambda p: (p.get("score_relevancia_v74", 0), p.get("score_editorial", 0)), reverse=True)
    except Exception:
        pass
    return resultado

_old_calcular_score_editorial_v74 = calcular_score_editorial

def calcular_score_editorial(pauta: dict) -> int:  # type: ignore[override]
    base = _old_calcular_score_editorial_v74(pauta)
    try:
        from ururau.coleta.relevancia_v74 import calcular_relevancia_v74
        rel = calcular_relevancia_v74(pauta)
        pauta.update(rel)
        # boost moderado: relevância não substitui score editorial, apenas prioriza manchete.
        base = max(0, min(100, int(base) + int(rel.get("score_relevancia_v74", 0)) // 5))
    except Exception:
        pass
    return base
