"""folha_adapter.py - Extração para Folha de S.Paulo e redir.folha.com.br."""
import re
import logging
from .helpers_v90 import (
    safe_get, get_soup, get_json_ld, find_json_ld_newsarticle,
    extract_paragraphs_from_soup, all_meaningful_paragraphs,
    titulo_from_soup, imagem_from_soup, legenda_from_soup,
    credito_from_soup, og_meta, fallback_result, build_result,
)

logger = logging.getLogger(__name__)
PREFIX = "[v90][ADAPTER][FOLHA]"


def extract(url: str, html: str = "") -> dict:
    result = fallback_result()
    result["metodo"] = "folha"
    result["motivo"] = "extração Folha de S.Paulo"

    soup = get_soup(html)
    if not soup:
        result["motivo"] = "HTML vazio ou inválido"
        return result

    # Detecta redir.folha.com.br
    is_redirect = "redir.folha.com.br" in url
    if is_redirect:
        logger.info(f"{PREFIX} Detectado redirect em {url}")

    titulo = ""
    texto = ""
    paragrafos = []
    imagem = None
    legenda = None
    credito = None
    metodo = "folha/desconhecido"
    aceito = False

    # 1. JSON-LD
    try:
        scripts = get_json_ld(soup)
        article = find_json_ld_newsarticle(scripts)
        if article:
            titulo = safe_get(article, "headline", "")
            texto = safe_get(article, "articleBody", "")
            img_data = safe_get(article, "image", {})
            imagem = safe_get(img_data, "url", None) if isinstance(img_data, dict) else (img_data if isinstance(img_data, str) else None)
            if titulo and texto:
                paragrafos = [p.strip() for p in texto.split("\n") if len(p.strip()) > 20]
                if not paragrafos:
                    paragrafos = [texto]
                aceito = True
                metodo = "folha/json-ld"
                logger.info(f"{PREFIX} JSON-LD ok titulo={titulo[:60]}")
    except Exception as e:
        logger.warning(f"{PREFIX} JSON-LD falhou: {e}")

    # 2. articleBody por itemprop
    if not aceito:
        try:
            body_el = soup.find(attrs={"itemprop": "articleBody"})
            if body_el:
                pars = extract_paragraphs_from_soup(body_el, None, min_len=30)
                if pars:
                    paragrafos = pars
                    texto = "\n\n".join(paragrafos)
                    aceito = True
                    metodo = "folha/articleBody"
                    logger.info(f"{PREFIX} articleBody ok")
        except Exception as e:
            logger.warning(f"{PREFIX} articleBody falhou: {e}")

    # 3. .content / .news / .c-news__body / .main-content
    if not aceito:
        for sel in (
            ".content",
            ".news",
            ".c-news__body",
            ".main-content",
            "article",
            ".article-content",
            ".texto",
        ):
            try:
                container = soup.select_one(sel)
                if container:
                    pars = extract_paragraphs_from_soup(container, None, min_len=30)
                    if len(pars) >= 2:
                        paragrafos = pars
                        texto = "\n\n".join(paragrafos)
                        aceito = True
                        metodo = f"folha/{sel}"
                        logger.info(f"{PREFIX} {sel} ok")
                        break
            except Exception as e:
                logger.warning(f"{PREFIX} {sel} falhou: {e}")

    # 4. Og:url para resolver redirect (log)
    if is_redirect and not aceito:
        try:
            og_url = og_meta(soup, "url")
            if og_url:
                logger.info(f"{PREFIX} og:url para redirect={og_url}")
        except Exception as e:
            logger.warning(f"{PREFIX} og:url falhou: {e}")

    # 5. Fallback parágrafos
    if not aceito:
        try:
            paragrafos = all_meaningful_paragraphs(soup, min_len=40)
            if paragrafos:
                texto = "\n\n".join(paragrafos)
                aceito = True
                metodo = "folha/fallback-p"
                logger.info(f"{PREFIX} fallback-p ok")
        except Exception as e:
            logger.warning(f"{PREFIX} fallback-p falhou: {e}")

    if not aceito or not paragrafos:
        result["motivo"] = "Não foi possível extrair conteúdo da Folha"
        return result

    if len(texto) < 150:
        result["motivo"] = f"Texto muito curto ({len(texto)} chars)"
        return result

    titulo = titulo or titulo_from_soup(soup, prefer_h1=True)
    imagem = imagem or imagem_from_soup(soup)
    legenda = legenda or legenda_from_soup(soup)
    credito = credito or credito_from_soup(soup)

    logger.info(f"{PREFIX} Aceito paragrafos={len(paragrafos)} metodo={metodo}")
    return build_result(
        True, titulo, texto, paragrafos,
        imagem, legenda, credito,
        metodo, "extraído via Folha adapter"
    )
