...
def _watchlists() -> dict:
    ...
    if _watchlists_cache is None:
        ...
        # v98: mescla termos editáveis do painel como grupo de watchlist.
        try:
            from ururau.coleta.termos_config_v98 import carregar_termos
            termos = carregar_termos()
            nomes = [t.get("termo", "") for t in termos if t.get("ativo", True) and t.get("termo")]
            peso_medio = 18
            if termos:
                try:
                    peso_medio = max(8, min(30, int(sum(int(t.get("peso", 18) or 18) for t in termos) / len(termos))))
                except Exception:
                    peso_medio = 18
            if nomes:
                _watchlists_cache["termos_config_painel_v98"] = {
                    "_peso": peso_medio,
                    "_descricao": "Termos editáveis da aba Config > Termos",
                    "nomes": nomes,
                }
        except Exception as e:
            print(f"[INTEL][TERMOS v98] falha ao carregar termos do painel: {e}")
...
def _score_temas_explosivos(texto_norm: str, wl: dict) -> tuple[int, str]:
    ...
    # v81: evita falso positivo de urgência em esporte.
    contexto_esporte = any(x in texto_norm for x in [
        "nba", "futebol", "basquete", "lakers", "rockets", "gol", "partida",
        "jogo", "campeonato", "playoffs", "time", "equipe"
    ])
    contexto_violencia = any(x in texto_norm for x in [
        "tiros", "morto", "ferido", "polícia", "policial", "atentado", "arma",
        "explosão", "desabamento", "vítima", "guerra", "ataque armado"
    ])
    for t in termos:
        if t in texto_norm:
            if contexto_esporte and not contexto_violencia:
                continue
...
def _watchlist_geral(texto_norm: str, wl: dict) -> tuple[int, list[str]]:
    ...
    for grupo_key in ["protagonistas_executivo_top_seo", "figuras_impacto_investigados", "termos_config_painel_v98"]:
    ...
    # v98: usa pesos individuais da aba Termos quando existirem.
    try:
        from ururau.coleta.termos_config_v98 import analisar_texto
        sc_extra, termos_extra, _canal = analisar_texto(texto_norm)
        if termos_extra:
            score += min(30, sc_extra)
            acionadas.extend([t for t in termos_extra if t not in acionadas][:4])
    except Exception:
        pass

    return min(score, 45), acionadas
